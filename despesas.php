...
<head>
    ...
    <title>IAFinance - Despesas</title>
    ...
    <script>
        tailwind.config = {
            darkMode: 'class',
            theme: {
                extend: { colors: { primary: '#3b82f6', secondary: '#10b981', danger: '#ef4444', dark: '#1f2937', darker: '#111827' } }
            }
        }
    </script>
</head>
<body class="bg-gray-100 dark:bg-darker text-gray-800 dark:text-gray-100 pb-24">
    ...
    <header class="bg-white dark:bg-dark shadow-sm sticky top-0 z-10 p-4 flex justify-between items-center">
        <h1 class="font-bold text-xl">Despesas 💸</h1>
        <button onclick="openModal()" class="bg-danger text-white px-4 py-2 rounded-lg text-sm font-bold shadow-lg shadow-red-500/30 hover:shadow-red-500/50 transition-all">+ Nova</button>
    </header>
    ...
    <div id="modal" class="fixed inset-0 bg-black/50 hidden z-50 flex items-end sm:items-center justify-center p-4 backdrop-blur-sm transition-all">
        <div class="bg-white dark:bg-dark w-full max-w-md rounded-2xl p-6 shadow-2xl transform transition-all translate-y-full sm:translate-y-0" id="modalContent">
            <div class="flex justify-between items-center mb-4">
                <h3 class="font-bold text-lg">Nova Despesa</h3>
            ...
            <form id="form" class="space-y-4">
                ...
                <button type="submit" class="w-full py-3.5 rounded-xl bg-danger text-white font-bold shadow-lg shadow-red-500/30">Salvar</button>
            </form>
        </div>
    </div>
    ...
    <script>
        ...
        async function loadItems() {
            ...
            const res = await fetch(`api/list_despesas.php?data=${date}&categoria=${cat}`);
            const data = await res.json();
            
            const list = document.getElementById('itemsList');
            list.innerHTML = '';

            if (data.length === 0) list.innerHTML = '<p class="text-center text-gray-400 mt-10">Nenhuma despesa encontrada.</p>';
            ...
            data.forEach(item => {
                ...
                el.innerHTML = `
                    <div class="flex items-center gap-3">
                        <div class="w-10 h-10 rounded-full bg-red-100 dark:bg-red-900/30 flex items-center justify-center text-red-600 font-bold">
                            💸
                        </div>
                        <div>
                    ...
                    <div class="text-right">
                        <p class="font-bold text-danger">- R$ ${parseFloat(item.valor).toFixed(2)}</p>
        ...
        async function deleteItem(id) {
            ...
            await fetch('api/delete_despesa.php', {
                method: 'POST',
        ...
        document.getElementById('form').addEventListener('submit', async (e) => {
            ...
            await fetch('api/create_despesa.php', {
                method: 'POST',
